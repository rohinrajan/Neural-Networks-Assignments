...
class ClNeuralNetwork:
    ...
    def adjust_weights(self, input_samples, error):
        ...
        if len(input_samples.shape) == 1:
            input_samples = np.append(input_values, 1)
        else:
            input_samples = np.vstack([input_samples, np.ones((1, input_samples.shape[1]), float)])
        ...
        new_error = np.dot(error, input_samples.T)
