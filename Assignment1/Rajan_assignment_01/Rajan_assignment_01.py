...
class CL_ActivationFunctions():
    ...
    def __init__(self, master):
		...
		# added the two options for hyperbolic tangent as well as positive linear
        self.learning_method_dropdown=Tk.OptionMenu(self.buttons_frame,self.learning_method_variable, "Sigmoid","Linear",
                                      "Hyperbolic Tangent","Positive Linear",command=lambda  event: self.learning_method_dropdown_callback())
    ...
    # This function is used to calculate the positive linear as per the definition 
    # given in text book if n < 0 then a = 0 else a = n (n>=0)
    def calculate_values(self,net_input):
        retValue = []
        for input in net_input:
            if input < 0:
                retValue.append(0)
            else:
                retValue.append(input)
        return retValue
    ...
    def display_activation_function(self):
        ...
        if self.learning_method=='Sigmoid':
            ...
            # the formula for hyperbolic Tangent is ((e^x - e ^-x) / (e^x + e^-x))
            activation = ((np.exp(net_value) - np.exp(-net_value))/(np.exp(net_value) + np.exp(-net_value)))
        elif self.learning_method=="Positive Linear":
            # if net_value is < 0 then return 0 else return net_value
            activation = self.calculate_values(net_value)
        self.axes.cla()
